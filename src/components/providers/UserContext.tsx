            ...
            await runTransaction(db, async (transaction) => {
                ...
                if (!docSnap.exists()) {
                    ...
                    if (isRepeatingGroup) {
                        ...
                        filteredExisting = existingAvailability.filter(
                            (event: Event) => event.repeatGroupId !== repeatGroupId
                        );
                    }

                    // Combine existing and new events, ensuring uniqueness based on date and time
                    const uniqueAvailability = [...filteredExisting, ...newAvailability]
                        .filter((event: Event, index: number, self: Event[]) => 
                            index === self.findIndex((e: Event) => 
