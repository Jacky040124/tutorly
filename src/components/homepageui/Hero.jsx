...
export function Hero() {
  ...
  return (
    <Container className="pb-16 pt-20 text-center lg:pt-32">
      ...
      <p className="mx-auto mt-6 max-w-2xl text-lg tracking-tight text-slate-700">{t("hero.subtitle")}</p>
          ...
          {[
            ...
            <li key={groupIndex}>
              <ul role="list" className="flex flex-col items-center gap-y-8 sm:flex-row sm:gap-x-12 sm:gap-y-0">
                {group.map((company) => (
                  <li key={company.name} className="flex flex-col items-center gap-y-2">
                    <Image
                      src={company.logo}
                      alt={t(company.name)}
                      width={48}
                      height={48}
                    />
                    <span className="text-sm text-slate-600">{t(company.name)}</span>
                  </li>
                ))}
              </ul>
            </li>
